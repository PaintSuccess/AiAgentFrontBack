...
async function getRecentOrdersForCustomer(customerId, limit = 3) {
  ...
  const data = await shopifyFetch(
    `orders.json?customer_id=${encodeURIComponent(customerId)}&status=any&limit=${limit}&order=created_at%20desc&fields=id,name,created_at,financial_status,fulfillment_status,total_price,currency,line_items`
  );
  return data.orders || [];
}
