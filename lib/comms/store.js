...
/**
 * Pin the ad a contact first arrived from, and log the touch.
 *
 * Meta sends the referral only on the first message of an ad-initiated conversation and
 * never again, so first_referral is written once and never overwritten — the `is null`
 * guard makes that hold even against a concurrent webhook. Every touch still lands in
 * `events`, so re-engagement from a later ad is not lost.
 */
async function recordAdReferral(sb, { contact, thread, message, input }) {
  await sb
    .from("contacts")
    .update({ first_referral: input.referral, first_referral_at: toIso(input.sentAt) })
    .eq("id", contact.id)
    .is("first_referral", null);
...
async function recordInbound(input) {
  try {
    ...
    const { message, isNew } = await insertMessage(sb, thread, contact.id, {
      ...
      metadata: input.referral
        ? { ...(input.metadata || {}), referral: input.referral }
        : input.metadata,
    });
    ...
    if (isNew) {
      ...
      if (input.referral) {
        // Attribution must never cost a customer their reply: keep its own guard so a
        // failure here (e.g. migration 0005 not yet applied) cannot abort recordInbound.
        // Loud on purpose — a silent miss is unrecoverable, the referral never re-sends.
        await recordAdReferral(sb, { contact, thread, message, input }).catch((err) =>
          console.error("[comms-store] AD REFERRAL LOST — capture failed:", err.message, JSON.stringify(input.referral))
        );
      }
    }
...
/**
 * Best-effort enrichment of a contact we already resolved (by phone).
 *
 * This exists so callers can attach identity WITHOUT passing it to resolveContact,
 * where `email` / `shopifyCustomerId` are match keys: a phone-directed message that
 * resolved via one of those could land on a contact with a DIFFERENT phone, and a
 * later staff reply from that thread would go to the wrong number.
 *
 * Only fills columns that are EMPTY — never overwrites. `email` and
 * `shopify_customer_id` are unique, so a value already owned by another contact is
 * skipped rather than stolen (e.g. a shared trade address like info@…).
 */
async function enrichContact(contactId, { email, shopifyCustomerId } = {}) {
  const sb = getSupabase();
  if (!sb || !contactId) return false;

  // One guarded UPDATE per column: `WHERE id = ? AND <col> IS NULL`. Doing this as a
  // read-then-write would race — two concurrent sends could both observe an empty
  // column and the later write would clobber the earlier one. The IS NULL filter makes
  // each fill atomic, so it is genuinely fill-only and never an overwrite.
  const fill = async (column, value) => {
    if (!value) return;
    try {
      const { error } = await sb
        ...
        .eq("id", contactId)
        .is(column, null);
      // 23505: another contact already owns this unique value (e.g. a shared trade
      // address). Leave both contacts alone rather than steal it.
      if (error && error.code !== UNIQUE_VIOLATION) throw error;
    } catch (err) {
      console.error(`[comms-store] enrichContact ${column} failed:`, err.message);
    }
  };

  await fill("email", normEmail(email));
  await fill("shopify_customer_id", shopifyCustomerId ? String(shopifyCustomerId) : null);
  return true;
}
