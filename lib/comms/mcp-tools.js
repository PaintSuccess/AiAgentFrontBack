...
async function commsSendWhatsAppTemplate(args = {}) {
  if (!args.approval_reference) {
    throw badRequest("approval_reference is required before sending a customer message.");
  }
  const templateRef = args.template_sid || args.template_name || args.template;
  if (!templateRef) throw badRequest("template_name (or template_sid) is required.");
  ...
  const result = await sendMessage({
    ...
    to,
    author: "human",
    template: { sid: args.template_sid, name: args.template_name || args.template, variables: args.variables || {} },
  });
  return {
    sent: true,
    channel: "whatsapp",
    to,
    template: result.template || templateRef,
    external_id: result.id || null,
    status: result.status || "sent",
    approval_reference: args.approval_reference,
  };
