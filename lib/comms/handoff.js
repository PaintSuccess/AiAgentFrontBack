...
async function escalateToHuman({ channel, phone, email, name, reason, preferred } = {}) {
  ...
  // The wa.me link is the customer's OWN route to a human, so it stands on its own.
  // With no link, a staff callback is the only route — if that page failed, claiming
  // "I've let our team know" is untrue, and pausing the AI on top would leave them in
  // silence with nobody coming.
  const reachable = Boolean(link) || staffNotified;
  ...
  if (method === "whatsapp") {
    if (!link) {
      ...
      // caller has no way to receive the link at all, so don't claim we sent it.
      const linkSent = await sendMessage({
        channel: "sms",
        to: toE164,
