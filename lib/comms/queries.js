...
/**
 * A contact's storefront browsing, for the inbox contact panel.
 *
 * Only events already stitched to this contact (`web_events.contact_id`) are returned —
 * anonymous `client_id`-only rows belong to nobody yet. `events` is the recent trail (newest
 * first); `summary` gives the at-a-glance numbers so the panel need not scan the list itself.
 */
async function getContactBrowsing(contactId, { limit = 20 } = {}) {
  const empty = { events: [], summary: { total: 0, products: 0, lastActive: null } };
  const sb = getSupabase();
  if (!sb || !contactId) return empty;

  const { data, error } = await sb
    .from("web_events")
    .select("name, url, product_id, product_title, data, occurred_at")
    .eq("contact_id", contactId)
    .order("occurred_at", { ascending: false })
    .limit(limit);
  if (error) {
    console.error("[queries] getContactBrowsing:", error.message);
    return empty;
  }

  const rows = data || [];
  // Count distinct products across the returned window, and surface last activity.
  ...
  return {
    events: rows.map((r) => ({
      ...
      query: r.data?.query || null, // search term, for search_submitted
      at: r.occurred_at,
    })),
    summary: {
      total: count ?? rows.length,
      products: new Set(rows.filter((r) => r.product_id).map((r) => r.product_id)).size,
      lastActive: rows[0]?.occurred_at || null,
    },
  };
}
