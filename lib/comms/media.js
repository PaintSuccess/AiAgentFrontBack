...
async function sendAssetOnWhatsApp({ to, assetKey, caption, author = "ai", contact = null }) {
  ...
  // Caption goes BOTH ways on purpose: the Twilio path (our default) only reads top-level
  // `Body`, while the Meta path reads `media.caption`. Passing only one drops it for the other.
  return send.sendMessage({
    channel: "whatsapp",
    to,
    body: caption || undefined,
    media: toSendMedia(asset, caption),
    author,
    contact,
  });
}
