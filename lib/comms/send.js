 ...
 */
/**
 * Identity safe to hand to store.resolveContact: `name` ONLY.
 *
 * `email` and `shopifyCustomerId` are MATCH KEYS in resolveContact. Passing them lets a
 * phone-directed message resolve to a contact with a DIFFERENT phone (a shared trade
 * address, or the customer's older number): the message is delivered to the number we
 * sent to, but logged under someone else's contact — and a later staff reply from that
 * thread is addressed from that contact, i.e. to the wrong person. So the phone stays
 * the only identifier that can select a contact here; the rest is applied afterwards by
 * store.enrichContact, which only fills empty columns.
 *
 * `name` is safe: resolveContact never matches on it, only back-fills it when empty.
 */
function contactIdentity(contact) {
  return contact?.name ? { name: contact.name } : {};
}

/** Attach non-routing identity to the contact the message was just logged against. */
async function enrichLoggedContact(logged, contact) {
  if (!logged?.contact?.id || !contact) return;
  if (!contact.email && !contact.shopifyCustomerId) return;
  await store
    .enrichContact(logged.contact.id, { email: contact.email, shopifyCustomerId: contact.shopifyCustomerId })
    .catch(() => {});
}

async function sendMessage({ channel, to, body, author = "human", media = null, template = null, contact = null }) {
  ...
  if (channel === "whatsapp" && template) {
    return sendWhatsAppTemplate({ toE164, template, author, contact });
  }
  ...
  await enrichLoggedContact(logged, contact);
...
async function sendWhatsAppTemplate({ toE164, template, author, contact = null }) {
...
module.exports = { sendMessage, twilioSendSms };
