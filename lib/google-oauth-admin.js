...
const DEFAULT_GOOGLE_PROJECT_SERVICES = ["gmail.googleapis.com", "drive.googleapis.com"];
...
function getGoogleCloudProjectId() {
  return cleanEnv("GOOGLE_CLOUD_PROJECT_ID") || cleanEnv("GOOGLE_PROJECT_ID") || cleanEnv("GCLOUD_PROJECT");
}

function getGoogleProjectServices() {
  const configured = cleanEnv("GOOGLE_PROJECT_SERVICES");
  if (!configured) return DEFAULT_GOOGLE_PROJECT_SERVICES;
  return configured
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean);
...
function verifyAdminPin(req) {
  ...
  // Fail CLOSED. This used to return ok:true when no PIN was configured, and it is the
  // ONLY gate on /api/google/oauth-start -- so with neither var set, anyone could walk
  // the flow with their own Google account and the callback would write THEIR
  // GOOGLE_REFRESH_TOKEN into production env, silently repointing every Gmail/Drive
  // operation at an attacker's mailbox. No PIN means no way to prove the caller is
  // staff, so the flow must stay shut rather than open.
  if (!configuredPin) return { ok: false, reason: "not_configured" };
...
async function enableGoogleProjectServices({ accessToken, projectId, services }) {
  if (!accessToken) {
    const err = new Error("Google access token is required to enable project services.");
    err.statusCode = 400;
    throw err;
  }
  if (!projectId) {
    const err = new Error("GOOGLE_CLOUD_PROJECT_ID is required to enable Google project services.");
    err.statusCode = 500;
    throw err;
  }

  const results = [];
  for (const service of services || DEFAULT_GOOGLE_PROJECT_SERVICES) {
    const url = `https://serviceusage.googleapis.com/v1/projects/${encodeURIComponent(
      projectId
    )}/services/${encodeURIComponent(service)}:enable`;
    ...
    const json = await res.json().catch(() => ({}));
    results.push({ service, ok: res.ok, status: res.status, response: json });
  }
  return { ok: results.every((item) => item.ok), projectId, results };
}
