...
function looksLikeDirectProductRequest(text) {
  const normalized = String(text || "").toLowerCase();
  if (
    /\b(link|links|url|urls|product|products|recommend|recommendation|price|stock|available|buy|order|sprayer|paint|kit|coating|roller|sander|primer|tape|brush|graco|dan|dans|rust|oleum|wagner|masking|floor|epoxy)\b/i.test(
      normalized
    )
  ) {
    return true;
  ...
  return new Promise((resolve, reject) => {
    ...
    async function finish(err, reply) {
      ...
      if (err) reject(err);
        ...
        if ((channel === "sms" || channel === "whatsapp") && PRODUCT_URL_RE.test(String(reply || ""))) {
