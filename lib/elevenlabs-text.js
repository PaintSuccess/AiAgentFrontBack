    ...
    async function finish(err, reply) {
      ...
      if (err) reject(err);
        ...
        const promisedToLook =
          /\b(let me|i'?ll|i will|i'?m)\s+(?:go |quickly |now )?(search|check|look|find|grab|pull)\w*/i.test(
            cleanReply
          );
        ...
        // Always hand the model's real reply to the formatter. It decides on the evidence
        // (empty / tool preface / on-screen reference) whether the reply is unusable and a
        // product summary should stand in — rather than a regex deciding up front that the
        // model's answer should be thrown away.
