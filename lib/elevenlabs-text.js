...
function collectProductToolResults(value, results = []) {
  ...
  if (typeof value === "string") {
    ...
    try {
      ...
      collectProductToolResults(parsed, results);
    } catch {}
    return results;
  }

  if (Array.isArray(value)) {
    for (const item of value) collectProductToolResults(item, results);
    return results;
  }
  ...
  for (const key of Object.keys(value)) {
    if (
      ...
      collectProductToolResults(value[key], results);
    }
  }
