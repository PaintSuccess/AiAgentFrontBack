...
function looksLikeDirectProductRequest(text) {
  const normalized = String(text || "").toLowerCase();
  if (
    /\b(link|links|url|urls|product|products|recommend|recommendation|price|stock|available|buy|sprayer|paint|kit|coating|roller|sander|primer|tape|brush|graco|dan|dans|rust|oleum|wagner|masking|floor|epoxy)\b/i.test(
      normalized
    )
  ) {
    return true;
  ...
  return new Promise((resolve, reject) => {
    ...
    async function finish(err, reply) {
      ...
      if (err) reject(err);
        ...
        const cleanReply = compactReply(reply);
        const textChannelProductIntent =
          (channel === "sms" || channel === "whatsapp") &&
          (looksLikeDirectProductRequest(text) ||
            looksLikeProductFollowUp(text) ||
            PRODUCT_URL_RE.test(String(reply || "")));

        if ((channel === "sms" || channel === "whatsapp") && !textChannelProductIntent) {
          productResult = null;
        }
        ...
        if (!cleanReply && (channel === "sms" || channel === "whatsapp")) {
