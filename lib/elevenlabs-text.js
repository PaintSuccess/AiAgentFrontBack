...
function mentionsAvailability(text) {
  return /\b(in stock|out of stock|unavailable|available|not showing in stock|stock updates?|sold out)\b/i.test(
    String(text || "")
  );
}
...
function formatTextChannelReply(reply, productToolResult, channel) {
  ...
  const stockNote = mentionsAvailability(text) ? "" : productAvailabilityNote(productToolResult);
  if (stockNote) {
    return compactReply(`${text}\n\n${stockNote}`);
  }
