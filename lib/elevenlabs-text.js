...
function looksLikeToolPreface(text) {
  const normalized = String(text || "").toLowerCase();
  return (
    /\b(let me|i'?ll|i will|just|no worries).{0,80}\b(search|check|look up|find|confirm|grab|get|send|connect).{0,30}\b(link|links|that|it|products?|stock|you)?\b/.test(normalized) ||
...
const REFERENTIAL_FOLLOWUP_RE =
  /\b(what about|which one|that one|this one|same one|other one|one too|those|these|same|other|asked|links?|urls?|grab|(?:first|second|third|last|black|white|red|blue|green|yellow|clear) one)\b/i;
  ...
  return new Promise((resolve, reject) => {
    ...
    async function finish(err, reply) {
      ...
      if (err) reject(err);
        ...
        // Only second-guess the model when the customer actually asked about products.
        // This block re-derives products by regex and can DISCARD the model's reply
        // (see forceProductSummary at the resolve below), so running it on a
        // non-product turn is how a discount-code question got answered with roller
        // covers. The guard above already nulls productResult for these turns; without
        // this condition the block simply put it back.
        if (isTextChannel && textChannelProductIntent) {
          ...
          // something we already showed. It used to also run whenever the direct search
          // came up empty, which is exactly when the turn is least likely to be about
          // products — dragging stale products from earlier in the thread into an
          // unrelated answer.
