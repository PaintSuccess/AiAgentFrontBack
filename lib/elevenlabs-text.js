    ...
    async function finish(err, reply) {
      ...
      if (err) {
        ...
        if (latestHandoffToolResult) {
          ...
          let handoffText;
          if (/wa\.me\//i.test(said)) handoffText = said;
          else if (!said || looksLikeToolPreface(said)) handoffText = compactReply(handoffMsg);
          else handoffText = compactReply(`${said}\n\n${handoffMsg}`);
          // Emit here too: this is the model successfully escalating via its own tool, and
          // it is precisely the escalation scenario we are trying to measure. Returning
          // without a line would quietly bias the data toward "the model never escalates".
          emitTurnTelemetry({
            ...telBase,
            ms: Date.now() - startedAt,
            outcome: "replied_handoff",
            modelConsulted: true,
            model: {
              responseParts: tel.responseParts,
              agentResponses: tel.agentResponses,
              ...
              webhookToolResponses: tel.webhookToolResponses,
              tools: tel.toolNames,
              replyChars: said.length,
            },
            us: { standInForHandoff: handoffText !== said, finalChars: handoffText.length },
          });
          resolve(handoffText);
          return;
        }
    ...
    ws.onopen = () => {
      sendJson(ws, {
        ...
        dynamic_variables: {
          ...
          // Preformatted first-name suffix (" Daniel" / ""), matching the widget's
          // `CFG.customerFirstName ? ' ' + CFG.customerFirstName : ''` and
          // baseDynamicVariables() on the phone path.
          //
          // This key must ALWAYS be present, even empty. Supplying a dynamic_variables
          // object that omits a {{variable}} referenced by the prompt or first_message
          // fails to resolve and the conversation never starts —
          // dynamic_variable_placeholders do NOT cover that case. Omitting it here is
          // precisely what took SMS/WhatsApp down on 2026-07-15.
          customer_greeting: customerName
