...
// PKCE is mandatory and S256-only (OAuth 2.1). Previously `if (!challenge) return true`
// let a client opt out of PKCE entirely — and since the challenge comes from the
// attacker's own authorization request, that was self-service. `plain` is refused too:
// it proves nothing once the challenge has been observed.
//
// This is also what contains authorization-code replay: codes are stateless signed
// payloads with no server-side consumption record, so a captured code could otherwise
// be exchanged repeatedly within its TTL. Requiring a verifier the attacker does not
// have makes a stolen code useless.
...
// Match the redirect URI by exact ORIGIN — never by prefix. This used to be a
// startsWith() test against a list that included "https://chatgpt.com" with no trailing
// slash, which made "https://chatgpt.com.evil.com/cb" a match: the authorization code
// would be redirected straight to an attacker-controlled host. The env var was already
// named ..._REDIRECT_ORIGINS; only the comparison was wrong.
...
function isAllowedRedirectUri(value) {
  ...
  const allowedOrigins = new Set(["https://chatgpt.com", "https://chat.openai.com", ...extra]);
