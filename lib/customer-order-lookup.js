...
// A "#" prefix is the only thing that marks a bare number as an order reference on
// sight. This catalogue is full of numeric model names — Graco 650, Graco 495, GH 200,
// Uni-Pro 230 — so an unmarked number must never be treated as an order number by
// itself, or "a service for a graco 650" turns into "I couldn't find order #650".
const EXPLICIT_ORDER_REF_RE = /#\s*\d{3,7}\b/;

function hasExplicitOrderReference(text) {
  return EXPLICIT_ORDER_REF_RE.test(String(text || ""));
}

// True when our own most recent message asked for order details. Only then is a bare
// number in the customer's reply ("44550") actually an answer to that question rather
// than a product model number mentioned in passing.
const ASKED_FOR_ORDER_DETAILS_RE = /\border number\b|\bemail used for that order\b/i;

function agentAskedForOrderDetails(conversationHistory = []) {
  const lastAgentTurn = [...(conversationHistory || [])]
    .reverse()
    .find((entry) => entry.role === "agent");
  return lastAgentTurn ? ASKED_FOR_ORDER_DETAILS_RE.test(lastAgentTurn.text || "") : false;
}
