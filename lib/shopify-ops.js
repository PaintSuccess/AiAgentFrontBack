...
function hasDuplicateOpsNote(existing, input = {}) {
  const note = String(existing || "");
  const requestId = safeText(input.request_id || input.idempotency_key, 120);
  if (requestId && note.includes(`Request id: ${requestId}`)) return true;
  ...
  return note.includes(`Type: ${type}`) && note.includes(`Source: ${source}`) && note.includes(`Summary: ${summary}`);
}
...
async function addOrderNote(input = {}) {
  ...
  if (hasDuplicateOpsNote(order.note, input)) {
    audit("shopify_add_order_note_duplicate", input, {
      order_id: order.id,
    ...
    return {
      ...
      order_id: order.id,
      order_number: order.order_number,
      message: "Matching Operations Desk note already exists; no duplicate note was added.",
    };
  }
...
async function completeFulfillment(input = {}) {
  ...
  if (!approvalReference) {
    const err = new Error("approval_reference is required before completing fulfilment.");
