<!-- ElevenLabs KB Doc | id: 8k7aHXxI2TAzsoLdui4x | usage_mode: auto -->
# Product Recommendation Details

Use this document only when the customer's question matches one of these product topics. For voice calls, summarize the relevant part in 1-3 sentences and ask one next question.

## DAN'S Spray / DAN'S Paint Spray

If a customer asks about "DAN'S Spray", "DAN'S Paint Spray", "Dance Spray", "Dan Spray", or similar wording, treat this as a likely request for the DAN'S cordless airless paint sprayer range.

Customers may pronounce DAN'S as "Dance". Recognize the intent naturally without correcting the customer directly.

Do not treat "DAN'S Spray" as one exact product name. Clarify briefly, then guide the customer toward the main DAN'S cordless models:

- DAN'S Airless Backpack
- DAN'S Compact
- DAN'S Wheeled

Voice-safe wording:

"Do you mean one of the DAN'S cordless airless sprayers, like the Backpack, Compact, or Wheeled model? What type of painting are you planning to do?"

Key points to mention only when relevant:

- Battery-powered and portable, so painters can work without a power cord.
- Good for cabinets, doors, trims, woodwork, fences, touch-ups, and small to medium painting jobs.
- Low-pressure airless setup helps with control, reduced overspray, and a smoother finish.
- Compatible with Graco tip guards and Graco spray tips.
- Gun and hose setup can be swapped or replaced when needed.

Recommended flow:

1. Clarify that the customer likely means the DAN'S cordless airless sprayer range.
2. Ask what they are painting and whether they prefer backpack, compact, or wheeled setup.
3. Use `search_products` before giving final product names, availability, prices, or product links.
4. Recommend the best available model based on job type, mobility needs, and setup preference.
5. In voice, do not list every feature. Give the top 1-2 reasons and ask the next question.

## Garage Floor Coatings & Concrete Paint

If a customer asks about garage floor coatings, garage floor paint, concrete floor paint, workshop floor paint, or similar products, do not tell them that all garage floor coatings are out of stock.

Explain that while some specialist garage floor coating systems may be unavailable, PaintAccess stocks an excellent Norglass alternative.

Recommended product:

Recommend the appropriate Norglass oil-based polyurethane floor coating.

Key points:

- Oil-based polyurethane coating.
- Self-priming.
- Suitable for bare concrete.
- Can also be applied over previously painted concrete, subject to compatibility.
- Durable, professional-quality finish suitable for garages, workshops, sheds, warehouses, and concrete floors.

Important application advice:

- If the floor has an existing coating, always recommend a small test patch first to check compatibility and adhesion.
- Surface must be clean, dry, and free from oil, grease, dust, loose material, and flaking paint.
- Proper preparation is critical for a durable, long-lasting finish.

Voice-safe wording:

"Some specialist garage floor systems may be unavailable, but Norglass has a good oil-based polyurethane floor coating option. Is your concrete bare, or has it already been painted?"

## Sprayer Recommendation Detail

For kitchen cabinets, furniture, doors, trims, windows, and high-end fine finish work, recommend HVLP or fine-finish spraying first.

For large walls, ceilings, fences, exteriors, roofs, commercial jobs, and bigger repaint projects, recommend airless sprayers.

For professional painters, recommend stronger professional airless machines such as Graco Ultra 390PC, 395PC, ProX17, ProX19, or higher-level systems depending on project size and confirmed Shopify availability.

For heavy coatings, CSR-style coatings, base coat, topcoat, elastomerics, block fillers, or thicker materials, do not promise compatibility automatically. Say that compatibility depends on the exact product, viscosity, tip size, and sprayer model, then check the data sheet or recommend a stronger unit.

## Paint Recommendation Detail

- Interior walls -> interior low-sheen or matt wall paint.
- Ceilings -> flat ceiling paint.
- Bathrooms, laundries, and mould-prone areas -> mould-resistant paint or suitable mould preparation products.
- Cabinets, trims, and doors -> enamel, 2-pack, or fine-finish coating depending on customer expectations and experience.
- Tiles, tubs, or specialty surfaces -> dedicated specialty systems where suitable and available.
- New plasterboard -> primer/sealer before topcoat.
- Stains, tannin, smoke, water marks, or difficult surfaces -> suitable primer/blocker such as Zinsser.

## Accessory Recommendation Detail

- Scraping old paint -> scrapers, blades, sanding tools, and dust control.
- Sanding large areas -> Mirka or dust-free sanding systems.
- Masking windows, floors, skirting, cabinets, or furniture -> masking tape, masking film, pretaped film, drop sheets, and protection products.
- Lived-in house or dust control -> ZipWall or dust barrier products.
- Sprayer purchase -> tips, filters, extension pole, hose, gun, cleaning fluid, pump armour, masking film, and PPE.

