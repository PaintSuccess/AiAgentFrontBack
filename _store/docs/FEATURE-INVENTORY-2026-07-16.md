...
| E7 | **Meta CAPI events** (`Lead`, `InitiateCheckout`, `Purchase`; `action_source: business_messaging`) | ❌ | Plan #2 specified `POST /api/meta/capi-event` — never built |
| E8 | Meta Pixel on storefront | ❓ | "need to verify" in May — **still unverified** |
