...
- Identify the Shopify order safely with `shopify_get_order` or `shopify_search_orders` from the PaintAccess Operations MCP.
- Confirm supplier mapping exists or ask for confirmation.
- Do not invent supplier emails or PO details.
- Use Gmail only through the PaintAccess Operations MCP backend Gmail tools; do not request or store Google credentials in chat or Git.

## Workflow

1. Retrieve the order through the PaintAccess Operations MCP.
